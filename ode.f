c    This file contains public-code routines for solving a system 
c    of ODEs using an Adams predictor-corrector algorithm.
c    
      subroutine ode(f,neqn,y,t,tout,relerr,abserr,iflag,work,iwork)
c
c   ---------------------------
c   Notes for Hydrolight (Feb 1998):
c   This is the SODE package, which was downloaded from NETLIB.
c
c   It is called by RICCATI [MAIN->RADAMPS->RICCATI->ODE]
c
c   It calls drtdzs (set by riccati as first (function) argument f)
c
c   The value of the machine-dependent constant u is computed in
c   Hydrolight routine SETDFLTS using the routine R1MACH and passed
c   to SODE routines DE and STEP via the common block CSODE.  Some 
c   local references to R1MACH and u have therefore been changed in 
c   the SODE routines. Otherwise, the SODE code is unchanged.
c   ---------------------------
c
